...
/// Read-through cache for GET responses, so a pharmacist with no signal
/// still sees their stock and order queue instead of an error screen.
///
...
///
/// Deliberately read-only. Queuing offline *writes* (dispensing, restocking)
/// needs server-side idempotency keys per action so a retry cannot
/// double-decrement stock — that is a backend change, not something to fake
...
class OfflineCache {
  ...
  /// use this device cannot read the previous pharmacist's stock offline.
...
class CachedEntry {
  ...
  /// so a pharmacist can judge whether the numbers are still trustworthy.
  String get ageLabel {
