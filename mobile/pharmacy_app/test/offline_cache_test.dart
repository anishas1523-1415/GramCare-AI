...
import 'package:pharmacy_mobile_app/services/offline_cache.dart';
...
void main() {
  ...
  test('read returns null when nothing was cached', () async {
    expect(await OfflineCache().read('stock'), isNull);
  });

  test('round-trips a cached list payload', () async {
    await OfflineCache().save('stock', [
      {'id': 1, 'patient_id': 7},
    ]);
    final entry = await OfflineCache().read('stock');
  ...
  test('clear removes cached entries so the next account cannot read them', () async {
    await OfflineCache().save('stock', [1, 2, 3]);
